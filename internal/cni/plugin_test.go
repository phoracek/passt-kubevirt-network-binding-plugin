...
var _ = Describe("passt-binding-plugin", func() {
	...
	Context("Add", func() {
		It("succeeds", func() {
			cmd := cni.NewCmd(stubNetNS{}, stubSysCtl{}, stubNetLink{})
			args := &skel.CmdArgs{
		...
		DescribeTable("fails to", func(sysCtl stubSysCtl, netLink stubNetLink, expectedErr error) {
			cmd := cni.NewCmd(stubNetNS{}, sysCtl, netLink)
			args := &skel.CmdArgs{
				ContainerID: "123456789",
				Netns:       testNSPath,
				IfName:      "",
	...
	It("Del", func() {
		Expect(cni.CmdDel(&skel.CmdArgs{})).To(Succeed())
	})

	It("Check", func() {
		Expect(cni.CmdCheck(&skel.CmdArgs{})).To(Succeed())
	})
})
